...
#include "ui_zhcacontroldialog.h"
...
std::list<std::string> ZHCAControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZHCAControlDialog::listMints;

ZHCAControlDialog::ZHCAControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZHCAControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZHCAControlDialog::~ZHCAControlDialog()
{
    delete ui;
}

void ZHCAControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZHCAControlDialog::updateList()
{
...
void ZHCAControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZHCAControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZHCA_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZHCAControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZHCAControlDialog::GetSelectedMints()
{
...
void ZHCAControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
