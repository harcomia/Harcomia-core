...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::HCA);
    ...
    if (!settings.contains("nAnonymizeHCAAmount"))
        settings.setValue("nAnonymizeHCAAmount", 1000);

    nAnonymizeHCAAmount = settings.value("nAnonymizeHCAAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeHCAAmount"))
        SoftSetArg("-anonymizehcaamount", settings.value("nAnonymizeHCAAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in hca.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHCAAmount:
            return QVariant(nAnonymizeHCAAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHCAAmount:
            nAnonymizeHCAAmount = value.toInt();
            settings.setValue("nAnonymizeHCAAmount", nAnonymizeHCAAmount);
            emit anonymizeHCAAmountChanged(nAnonymizeHCAAmount);
            break;
        case CoinControlFeatures:
