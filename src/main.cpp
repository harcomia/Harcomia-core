...
#if defined(NDEBUG)
#error "HCA cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zHCA spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zHCA spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zHCA spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight >= 201 && nHeight <= 43200) { // phase 1
        nSubsidy = 0.33 * COIN;
    }
	else if (nHeight >= 43201 && nHeight <= 100800) { // phase 2
        nSubsidy = 0.35 * COIN;
    }
	else if (nHeight >= 100801 && nHeight <= 141121) { // phase 3
        nSubsidy = 0.37 * COIN;
    }
	else if (nHeight >= 141122 && nHeight <= 191522) { // phase 4
        nSubsidy = 0.40 * COIN;
    }
	else if (nHeight >= 191523 && nHeight <= 231843) { // phase 5
        nSubsidy = 0.45 * COIN;
    }
	else if (nHeight >= 231844 && nHeight <= 262084) { // phase 6
        nSubsidy = 0.50 * COIN;
    }
	else if (nHeight >= 262085 && nHeight <= 272165) { // phase 7
        nSubsidy = 0.55 * COIN;
    }
	else if (nHeight >= 272166 && nHeight <= 282246) { // phase 8
        nSubsidy = 0.60 * COIN;
    }
	else if (nHeight >= 282247 && nHeight <= 292327) { // phase 9
        nSubsidy = 0.62 * COIN;
    }
	else if (nHeight >= 292328 && nHeight <= 302408) { // phase 10
        nSubsidy = 0.65 * COIN;
    }
	else if (nHeight >= 302409 && nHeight <= 312489) { // phase 11
        nSubsidy = 0.67 * COIN;
    }
	else if (nHeight >= 312490 && nHeight <= 322570) { // phase 12
        nSubsidy = 0.70 * COIN;
    }
	else if (nHeight >= 322571 && nHeight <= 332651) { // phase 13
        nSubsidy = 0.73 * COIN;
    }
	else if (nHeight >= 332652 && nHeight <= 342732) { // phase 14
        nSubsidy = 0.75 * COIN;
    }
	else if (nHeight >= 342733 && nHeight <= 352813) { // phase 15
        nSubsidy = 0.77 * COIN;
    }
	else if (nHeight >= 352814 && nHeight <= 362894) { // phase 16
        nSubsidy = 0.79 * COIN;
    }
	else if (nHeight >= 362895 && nHeight <= 372975) { // phase 17
        nSubsidy = 0.82 * COIN;
    }
	else if (nHeight >= 372976 && nHeight <= 383056) { // phase 18
        nSubsidy = 0.85 * COIN;
    }
	else if (nHeight >= 383057 && nHeight <= 393137) { // phase 19
        nSubsidy = 0.87 * COIN;
    }
	else if (nHeight >= 393138 && nHeight <= 433458) { // phase 20
        nSubsidy = 0.90 * COIN;
    }
	else if (nHeight >= 433459 && nHeight <= 473779) { // phase 21
        nSubsidy = 0.95 * COIN;
    }
	else if (nHeight >= 473780 && nHeight <= 514100) { // phase 22
        nSubsidy = 0.90 * COIN;
    }
	else if (nHeight >= 514101 && nHeight <= 554421) { // phase 23
        nSubsidy = 0.80 * COIN;
    }
	else if (nHeight >= 554422 && nHeight <= 594742) { // phase 24
        nSubsidy = 0.70 * COIN;
    }
	else if (nHeight >= 594743 && nHeight <= 635063) { // phase 25
        nSubsidy = 0.60 * COIN;
    }
	else if (nHeight >= 635064 && nHeight <= 675384) { // phase 26
        nSubsidy = 0.50 * COIN;
    }
	else if (nHeight >= 675385 && nHeight <= 715705) { // phase 27
        nSubsidy = 0.45 * COIN;
    }
	else if (nHeight >= 715706 && nHeight <= 756026) { // phase 28
        nSubsidy = 0.40 * COIN;
    }
	else if (nHeight >= 756027 && nHeight <= 796347) { // phase 29
        nSubsidy = 0.37 * COIN;
    }	
	else if (nHeight >= 796348 && nHeight <= 836668) { // phase 30
        nSubsidy = 0.35 * COIN;
    }
	else if (nHeight >= 836669 && nHeight <= 22960000) { // phase 31
        nSubsidy = 0.20 * COIN;
    }	
	else if (nHeight > 22960001) {
        nSubsidy = 0.00 * COIN;
    }
    else {
        nSubsidy = 0.2 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from HCA
...
{
    RenameThread("hca-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZHCAMinted()
{
...
void RecalculateZHCASpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zHCA supply
...
bool RecalculateHCASupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zHCA with serial %s is already in the block %d\n",
    ...
    if (!fVerifyingBlocks && pindex->nHeight == Params().Zerocoin_StartHeight() + 1) {
        RecalculateZHCAMinted();
        RecalculateZHCASpent();
        RecalculateHCASupply(1);
    }
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zHCASpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert HCA to zHCA
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zHCA spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zHCA serial %s in block\n Block: %s",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if(IsSporkActive(SPORK_20_SPAM_CHK) && (block.GetBlockTime() >= GetSporkValue(SPORK_20_SPAM_CHK))) {
      ...
      if (block.IsProofOfStake()) {
          ...
          // Inputs
          std::vector<CTxIn> hcaInputs;
          std::vector<CTxIn> zHCAInputs;

          for (CTxIn stakeIn : stakeTxIn.vin) {
              if(stakeIn.scriptSig.IsZerocoinSpend()){
                  zHCAInputs.push_back(stakeIn);
              }else{
                  hcaInputs.push_back(stakeIn);
              }
          }
          const bool hasHCAInputs = !hcaInputs.empty();
          const bool hasZHCAInputs = !zHCAInputs.empty();
          ...
          for (CTransaction tx : block.vtx) {
              for (CTxIn in: tx.vin) {
                  ...
                  if(hasHCAInputs)
                      ...
                      for (CTxIn hcaIn : hcaInputs){
                          if(hcaIn.prevout == in.prevout){
          ...
          if (isBlockFromFork) {
              ...
              // Now that this loop if completed. Check if we have zHCA inputs.
              if(hasZHCAInputs){
                  for (CTxIn zPawsInput : zHCAInputs) {
                      ...
                      //                                stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zhca");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zHCA mints and %d zHCA spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // HCA: We use certain sporks during IBD, so check to see if they are
...
{
	return CScript() << ParseHex("0428bc7a04d1a0dd673d8ba05e6c05e06354c0c16a41492c65c8dc566c9b5eb5bd2001cd45cf75c57e4bfafacc6ca9648b32af87ef95fe4d356bfa9bdaebc9ebb6") << OP_CHECKSIG;
}
